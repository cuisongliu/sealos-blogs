...
- [x] 1.6 添加 GitHub Actions 通过 Launchpad API（POST /api/v1/app）管理应用的示例，明确凭据注入、最小权限与请求体模板
- [x] 1.7 补充安全注意事项：权限最小化、有效期、吊销与旋转处理，以及凭据丢失的应对
- [x] 1.8 添加常见问题与排查（证书过期、无权限、网络连通性），给出可操作的检查步骤
- [x] 1.9 在 README 或文档索引处添加 kubeconfig 指南的入口链接，确保易发现
